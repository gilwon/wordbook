import type { Word } from '@/lib/types';

export const wordsEs: Word[] = [
  {
    id: "es-001",
    language: "es",
    word: "alegría",
    phonetic: "/a.leˈɣɾi.a/",
    pronunciationKo: "알레그리아",
    meaningKo: "기쁨",
    partOfSpeech: "명사",
    explanationKo: "행복하고 기쁜 감정 상태",
    example: "La alegría de los niños es contagiosa.",
    examplePronunciationKo: "라 알레그리아 데 로스 니뇨스 에스 콘타히오사.",
    exampleKo: "아이들의 기쁨은 전염성이 있다.",
    exampleSegments: [
      { original: "La alegría de los niños ", ko: "아이들의 기쁨은 " },
      { original: "es contagiosa.", ko: "전염성이 있다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-002",
    language: "es",
    word: "bondad",
    phonetic: "/bonˈdað/",
    pronunciationKo: "본다드",
    meaningKo: "친절함",
    partOfSpeech: "명사",
    explanationKo: "선하고 친절한 성품이나 행동",
    example: "Su bondad nos conmovió profundamente.",
    examplePronunciationKo: "수 본다드 노스 콘모비오 프로푼다멘테.",
    exampleKo: "그의 친절함이 우리를 깊이 감동시켰다.",
    exampleSegments: [
      { original: "Su bondad ", ko: "그의 친절함이 " },
      { original: "nos conmovió profundamente.", ko: "우리를 깊이 감동시켰다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-003",
    language: "es",
    word: "caminar",
    phonetic: "/ka.miˈnaɾ/",
    pronunciationKo: "카미나르",
    meaningKo: "걷다",
    partOfSpeech: "동사",
    explanationKo: "발로 한 걸음씩 이동하다",
    example: "Me gusta caminar por el parque cada mañana.",
    examplePronunciationKo: "메 구스타 카미나르 포르 엘 파르케 카다 마냐나.",
    exampleKo: "나는 매일 아침 공원을 걷는 것을 좋아한다.",
    exampleSegments: [
      { original: "Me gusta caminar por el parque cada mañana.", ko: "나는 매일 아침 공원을 걷는 것을 좋아한다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-004",
    language: "es",
    word: "descansar",
    phonetic: "/des.kanˈsaɾ/",
    pronunciationKo: "데스칸사르",
    meaningKo: "쉬다",
    partOfSpeech: "동사",
    explanationKo: "피로를 회복하기 위해 활동을 멈추다",
    example: "Necesito descansar un poco después del trabajo.",
    examplePronunciationKo: "네세시토 데스칸사르 운 포코 데스푸에스 델 트라바호.",
    exampleKo: "일이 끝난 후 좀 쉬어야 한다.",
    exampleSegments: [
      { original: "Necesito descansar un poco después del trabajo.", ko: "일이 끝난 후 좀 쉬어야 한다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-005",
    language: "es",
    word: "esperar",
    phonetic: "/es.peˈɾaɾ/",
    pronunciationKo: "에스페라르",
    meaningKo: "기다리다",
    partOfSpeech: "동사",
    explanationKo: "무언가가 오거나 일어나기를 기다리다",
    example: "Voy a esperar aquí hasta que llegues.",
    examplePronunciationKo: "보이 아 에스페라르 아키 아스타 케 예게스.",
    exampleKo: "네가 도착할 때까지 여기서 기다릴게.",
    exampleSegments: [
      { original: "Voy a esperar aquí hasta que llegues.", ko: "네가 도착할 때까지 여기서 기다릴게." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-006",
    language: "es",
    word: "felicidad",
    phonetic: "/fe.li.siˈðað/",
    pronunciationKo: "펠리시다드",
    meaningKo: "행복",
    partOfSpeech: "명사",
    explanationKo: "삶에서 만족과 기쁨을 느끼는 상태",
    example: "La felicidad está en las pequeñas cosas.",
    examplePronunciationKo: "라 펠리시다드 에스타 엔 라스 페케냐스 코사스.",
    exampleKo: "행복은 작은 것에 있다.",
    exampleSegments: [
      { original: "La felicidad está en las pequeñas cosas.", ko: "행복은 작은 것에 있다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-007",
    language: "es",
    word: "hermoso",
    phonetic: "/eɾˈmo.so/",
    pronunciationKo: "에르모소",
    meaningKo: "아름다운",
    partOfSpeech: "형용사",
    explanationKo: "보기에 매우 아름답고 매력적인",
    example: "El paisaje de la montaña es hermoso.",
    examplePronunciationKo: "엘 파이사헤 데 라 몬타냐 에스 에르모소.",
    exampleKo: "산의 풍경이 아름답다.",
    exampleSegments: [
      { original: "El paisaje de la montaña ", ko: "산의 풍경이 " },
      { original: "es hermoso.", ko: "아름답다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-008",
    language: "es",
    word: "importante",
    phonetic: "/im.poɾˈtan.te/",
    pronunciationKo: "임포르탄테",
    meaningKo: "중요한",
    partOfSpeech: "형용사",
    explanationKo: "큰 가치나 의미를 가지고 있는",
    example: "Es importante estudiar todos los días.",
    examplePronunciationKo: "에스 임포르탄테 에스투디아르 토도스 로스 디아스.",
    exampleKo: "매일 공부하는 것이 중요하다.",
    exampleSegments: [
      { original: "Es importante estudiar todos los días.", ko: "매일 공부하는 것이 중요하다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-009",
    language: "es",
    word: "jugar",
    phonetic: "/xuˈɣaɾ/",
    pronunciationKo: "후가르",
    meaningKo: "놀다",
    partOfSpeech: "동사",
    explanationKo: "즐겁게 놀거나 경기를 하다",
    example: "Los niños juegan en el parque cada tarde.",
    examplePronunciationKo: "로스 니뇨스 후에간 엔 엘 파르케 카다 타르데.",
    exampleKo: "아이들은 매일 오후 공원에서 논다.",
    exampleSegments: [
      { original: "Los niños juegan en el parque cada tarde.", ko: "아이들은 매일 오후 공원에서 논다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-010",
    language: "es",
    word: "libertad",
    phonetic: "/li.βeɾˈtað/",
    pronunciationKo: "리베르타드",
    meaningKo: "자유",
    partOfSpeech: "명사",
    explanationKo: "원하는 대로 행동하거나 생각할 수 있는 권리",
    example: "La libertad es un derecho fundamental.",
    examplePronunciationKo: "라 리베르타드 에스 운 데레초 푼다멘탈.",
    exampleKo: "자유는 기본적인 권리이다.",
    exampleSegments: [
      { original: "La libertad ", ko: "자유는 " },
      { original: "es un derecho fundamental.", ko: "기본적인 권리이다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-011",
    language: "es",
    word: "maravilloso",
    phonetic: "/ma.ɾa.βiˈʎo.so/",
    pronunciationKo: "마라비요소",
    meaningKo: "놀라운",
    partOfSpeech: "형용사",
    explanationKo: "매우 인상적이고 감탄스러운",
    example: "Fue un viaje maravilloso lleno de aventuras.",
    examplePronunciationKo: "푸에 운 비아헤 마라비요소 예노 데 아벤투라스.",
    exampleKo: "모험으로 가득한 놀라운 여행이었다.",
    exampleSegments: [
      { original: "Fue un viaje maravilloso lleno de aventuras.", ko: "모험으로 가득한 놀라운 여행이었다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-012",
    language: "es",
    word: "naturaleza",
    phonetic: "/na.tu.ɾaˈle.sa/",
    pronunciationKo: "나투랄레사",
    meaningKo: "자연",
    partOfSpeech: "명사",
    explanationKo: "동식물과 지구의 자연 환경",
    example: "Me encanta pasar tiempo en la naturaleza.",
    examplePronunciationKo: "메 엔칸타 파사르 티엠포 엔 라 나투랄레사.",
    exampleKo: "나는 자연 속에서 시간을 보내는 것을 좋아한다.",
    exampleSegments: [
      { original: "Me encanta pasar tiempo en la naturaleza.", ko: "나는 자연 속에서 시간을 보내는 것을 좋아한다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-013",
    language: "es",
    word: "paciencia",
    phonetic: "/pa.ˈsjen.sja/",
    pronunciationKo: "파시엔시아",
    meaningKo: "인내심",
    partOfSpeech: "명사",
    explanationKo: "어려운 상황을 평온하게 견디는 능력",
    example: "Necesitas paciencia para aprender un idioma.",
    examplePronunciationKo: "네세시타스 파시엔시아 파라 아프렌데르 운 이디오마.",
    exampleKo: "언어를 배우려면 인내심이 필요하다.",
    exampleSegments: [
      { original: "Necesitas paciencia para aprender un idioma.", ko: "언어를 배우려면 인내심이 필요하다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-014",
    language: "es",
    word: "recordar",
    phonetic: "/re.koɾˈðaɾ/",
    pronunciationKo: "레코르다르",
    meaningKo: "기억하다",
    partOfSpeech: "동사",
    explanationKo: "과거의 것을 마음속에서 다시 떠올리다",
    example: "Siempre recordaré ese hermoso momento.",
    examplePronunciationKo: "시엠프레 레코르다레 에세 에르모소 모멘토.",
    exampleKo: "나는 그 아름다운 순간을 항상 기억할 것이다.",
    exampleSegments: [
      { original: "Siempre recordaré ese hermoso momento.", ko: "나는 그 아름다운 순간을 항상 기억할 것이다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-015",
    language: "es",
    word: "sueño",
    phonetic: "/ˈswe.ɲo/",
    pronunciationKo: "수에뇨",
    meaningKo: "꿈",
    partOfSpeech: "명사",
    explanationKo: "자는 동안 꾸는 꿈 또는 미래의 희망",
    example: "Tuve un sueño muy hermoso anoche.",
    examplePronunciationKo: "투베 운 수에뇨 무이 에르모소 아노체.",
    exampleKo: "어젯밤 매우 아름다운 꿈을 꿨다.",
    exampleSegments: [
      { original: "Tuve un sueño muy hermoso anoche.", ko: "어젯밤 매우 아름다운 꿈을 꿨다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-016",
    language: "es",
    word: "tranquilo",
    phonetic: "/tɾaŋˈki.lo/",
    pronunciationKo: "트랑킬로",
    meaningKo: "조용한",
    partOfSpeech: "형용사",
    explanationKo: "평화롭고 아무런 소란이 없는",
    example: "Este lugar es muy tranquilo y relajante.",
    examplePronunciationKo: "에스테 루가르 에스 무이 트랑킬로 이 렐라한테.",
    exampleKo: "이 곳은 매우 조용하고 편안하다.",
    exampleSegments: [
      { original: "Este lugar es ", ko: "이 곳은 " },
      { original: "muy tranquilo ", ko: "매우 조용하고 " },
      { original: "y relajante.", ko: "편안하다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-017",
    language: "es",
    word: "valiente",
    phonetic: "/baˈljɛn.te/",
    pronunciationKo: "발리엔테",
    meaningKo: "용감한",
    partOfSpeech: "형용사",
    explanationKo: "위험을 두려워하지 않고 맞서는",
    example: "El bombero es valiente y siempre ayuda.",
    examplePronunciationKo: "엘 봄베로 에스 발리엔테 이 시엠프레 아유다.",
    exampleKo: "소방관은 용감하고 항상 도움을 준다.",
    exampleSegments: [
      { original: "El bombero es valiente ", ko: "소방관은 용감하고 " },
      { original: "y siempre ayuda.", ko: "항상 도움을 준다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-018",
    language: "es",
    word: "aprender",
    phonetic: "/a.pɾenˈdeɾ/",
    pronunciationKo: "아프렌데르",
    meaningKo: "배우다",
    partOfSpeech: "동사",
    explanationKo: "공부나 경험을 통해 새로운 것을 알게 되다",
    example: "Me encanta aprender nuevos idiomas.",
    examplePronunciationKo: "메 엔칸타 아프렌데르 누에보스 이디오마스.",
    exampleKo: "나는 새로운 언어를 배우는 것을 좋아한다.",
    exampleSegments: [
      { original: "Me encanta aprender nuevos idiomas.", ko: "나는 새로운 언어를 배우는 것을 좋아한다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-019",
    language: "es",
    word: "brillante",
    phonetic: "/bɾiˈʎan.te/",
    pronunciationKo: "브리얀테",
    meaningKo: "빛나는",
    partOfSpeech: "형용사",
    explanationKo: "밝게 빛나거나 매우 뛰어난 능력을 가진",
    example: "Tiene una mente brillante para las matemáticas.",
    examplePronunciationKo: "티에네 우나 멘테 브리얀테 파라 라스 마테마티카스.",
    exampleKo: "그는 수학에서 빛나는 두뇌를 가지고 있다.",
    exampleSegments: [
      { original: "Tiene una mente brillante para las matemáticas.", ko: "그는 수학에서 빛나는 두뇌를 가지고 있다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-020",
    language: "es",
    word: "disfrutar",
    phonetic: "/dis.fɾuˈtaɾ/",
    pronunciationKo: "디스프루타르",
    meaningKo: "즐기다",
    partOfSpeech: "동사",
    explanationKo: "기쁨과 즐거움을 느끼다",
    example: "Me gusta disfrutar de la música clásica.",
    examplePronunciationKo: "메 구스타 디스프루타르 데 라 무시카 클라시카.",
    exampleKo: "나는 클래식 음악을 즐기는 것을 좋아한다.",
    exampleSegments: [
      { original: "Me gusta disfrutar de la música clásica.", ko: "나는 클래식 음악을 즐기는 것을 좋아한다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-021",
    language: "es",
    word: "energía",
    phonetic: "/e.neɾˈxi.a/",
    pronunciationKo: "에네르히아",
    meaningKo: "에너지",
    partOfSpeech: "명사",
    explanationKo: "활동하는 데 필요한 힘과 활력",
    example: "Los niños tienen mucha energía para jugar.",
    examplePronunciationKo: "로스 니뇨스 티에넨 무차 에네르히아 파라 후가르.",
    exampleKo: "아이들은 놀기 위한 에너지가 넘친다.",
    exampleSegments: [
      { original: "Los niños tienen mucha energía para jugar.", ko: "아이들은 놀기 위한 에너지가 넘친다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-022",
    language: "es",
    word: "generoso",
    phonetic: "/xe.neˈɾo.so/",
    pronunciationKo: "헤네로소",
    meaningKo: "관대한",
    partOfSpeech: "형용사",
    explanationKo: "아낌없이 베풀고 주는",
    example: "Es una persona muy generosa con todos.",
    examplePronunciationKo: "에스 우나 페르소나 무이 헤네로사 콘 토도스.",
    exampleKo: "그는 모두에게 매우 관대한 사람이다.",
    exampleSegments: [
      { original: "Es una persona muy generosa con todos.", ko: "그는 모두에게 매우 관대한 사람이다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-023",
    language: "es",
    word: "honesto",
    phonetic: "/oˈnes.to/",
    pronunciationKo: "오네스토",
    meaningKo: "정직한",
    partOfSpeech: "형용사",
    explanationKo: "거짓 없이 진실되고 도덕적인",
    example: "Sé honesto con tus propios sentimientos.",
    examplePronunciationKo: "세 오네스토 콘 투스 프로피오스 센티미엔토스.",
    exampleKo: "자신의 감정에 정직해라.",
    exampleSegments: [
      { original: "Sé honesto con tus propios sentimientos.", ko: "자신의 감정에 정직해라." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-024",
    language: "es",
    word: "conocer",
    phonetic: "/ko.noˈseɾ/",
    pronunciationKo: "코노세르",
    meaningKo: "알다",
    partOfSpeech: "동사",
    explanationKo: "누군가를 알거나 어떤 장소를 방문하다",
    example: "Me gustaría conocer nuevas personas.",
    examplePronunciationKo: "메 구스타리아 코노세르 누에바스 페르소나스.",
    exampleKo: "나는 새로운 사람들을 알고 싶다.",
    exampleSegments: [
      { original: "Me gustaría conocer nuevas personas.", ko: "나는 새로운 사람들을 알고 싶다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-025",
    language: "es",
    word: "cuidar",
    phonetic: "/kwiˈðaɾ/",
    pronunciationKo: "쿠이다르",
    meaningKo: "돌보다",
    partOfSpeech: "동사",
    explanationKo: "누군가나 무언가를 보살피고 관리하다",
    example: "Debemos cuidar el medio ambiente.",
    examplePronunciationKo: "데베모스 쿠이다르 엘 메디오 암비엔테.",
    exampleKo: "우리는 환경을 돌봐야 한다.",
    exampleSegments: [
      { original: "Debemos cuidar el medio ambiente.", ko: "우리는 환경을 돌봐야 한다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-026",
    language: "es",
    word: "fuerza",
    phonetic: "/ˈfweɾ.sa/",
    pronunciationKo: "푸에르사",
    meaningKo: "힘",
    partOfSpeech: "명사",
    explanationKo: "신체적 또는 정신적인 힘과 능력",
    example: "Necesitas fuerza para superar los obstáculos.",
    examplePronunciationKo: "네세시타스 푸에르사 파라 수페라르 로스 옵스타쿨로스.",
    exampleKo: "장애물을 극복하려면 힘이 필요하다.",
    exampleSegments: [
      { original: "Necesitas fuerza para superar los obstáculos.", ko: "장애물을 극복하려면 힘이 필요하다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-027",
    language: "es",
    word: "olvidar",
    phonetic: "/ol.βiˈðaɾ/",
    pronunciationKo: "올비다르",
    meaningKo: "잊다",
    partOfSpeech: "동사",
    explanationKo: "기억에서 사라지거나 기억하지 못하다",
    example: "No puedo olvidar ese día tan especial.",
    examplePronunciationKo: "노 푸에도 올비다르 에세 디아 탄 에스페시알.",
    exampleKo: "그 특별했던 날을 잊을 수 없다.",
    exampleSegments: [
      { original: "No puedo olvidar ese día tan especial.", ko: "그 특별했던 날을 잊을 수 없다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-028",
    language: "es",
    word: "querer",
    phonetic: "/keˈɾeɾ/",
    pronunciationKo: "케레르",
    meaningKo: "사랑하다",
    partOfSpeech: "동사",
    explanationKo: "누군가를 깊이 사랑하거나 무언가를 원하다",
    example: "Te quiero mucho, más que a nada.",
    examplePronunciationKo: "테 키에로 무초, 마스 케 아 나다.",
    exampleKo: "무엇보다 너를 많이 사랑해.",
    exampleSegments: [
      { original: "Te quiero mucho, más que a nada.", ko: "무엇보다 너를 많이 사랑해." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-029",
    language: "es",
    word: "único",
    phonetic: "/ˈu.ni.ko/",
    pronunciationKo: "우니코",
    meaningKo: "유일한",
    partOfSpeech: "형용사",
    explanationKo: "세상에서 오직 하나뿐인 특별한",
    example: "Eres único en este mundo tan grande.",
    examplePronunciationKo: "에레스 우니코 엔 에스테 문도 탄 그란데.",
    exampleKo: "넌 이 넓은 세상에서 유일한 존재야.",
    exampleSegments: [
      { original: "Eres único en este mundo tan grande.", ko: "넌 이 넓은 세상에서 유일한 존재야." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-030",
    language: "es",
    word: "crecer",
    phonetic: "/kɾeˈseɾ/",
    pronunciationKo: "크레세르",
    meaningKo: "성장하다",
    partOfSpeech: "동사",
    explanationKo: "크기나 능력이 점점 커지고 발전하다",
    example: "Los niños crecen muy rápido.",
    examplePronunciationKo: "로스 니뇨스 크레센 무이 라피도.",
    exampleKo: "아이들은 매우 빠르게 성장한다.",
    exampleSegments: [
      { original: "Los niños crecen muy rápido.", ko: "아이들은 매우 빠르게 성장한다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-031",
    language: "es",
    word: "rutina",
    phonetic: "/ruˈti.na/",
    pronunciationKo: "루티나",
    meaningKo: "일과",
    partOfSpeech: "명사",
    explanationKo: "매일 반복하는 습관적인 활동의 흐름",
    example: "Tengo una rutina estricta cada mañana.",
    examplePronunciationKo: "텡고 우나 루티나 에스트릭타 카다 마냐나.",
    exampleKo: "나는 매일 아침 엄격한 일과가 있다.",
    exampleSegments: [
      { original: "Tengo una rutina estricta cada mañana.", ko: "나는 매일 아침 엄격한 일과가 있다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-032",
    language: "es",
    word: "vecindario",
    phonetic: "/be.sinˈda.ɾjo/",
    pronunciationKo: "베신다리오",
    meaningKo: "동네",
    partOfSpeech: "명사",
    explanationKo: "사람들이 함께 사는 지역이나 구역",
    example: "Vivo en un vecindario muy tranquilo.",
    examplePronunciationKo: "비보 엔 운 베신다리오 무이 트랑킬로.",
    exampleKo: "나는 매우 조용한 동네에 산다.",
    exampleSegments: [
      { original: "Vivo en un vecindario muy tranquilo.", ko: "나는 매우 조용한 동네에 산다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-033",
    language: "es",
    word: "tarea",
    phonetic: "/taˈɾe.a/",
    pronunciationKo: "타레아",
    meaningKo: "집안일",
    partOfSpeech: "명사",
    explanationKo: "집에서 정기적으로 해야 하는 가사 작업",
    example: "Lavar los platos es mi tarea menos favorita.",
    examplePronunciationKo: "라바르 로스 플라토스 에스 미 타레아 메노스 파보리타.",
    exampleKo: "설거지는 내가 가장 싫어하는 집안일이다.",
    exampleSegments: [
      { original: "Lavar los platos ", ko: "설거지는 " },
      { original: "es mi tarea menos favorita.", ko: "내가 가장 싫어하는 집안일이다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-034",
    language: "es",
    word: "presupuesto",
    phonetic: "/pɾe.suˈpwes.to/",
    pronunciationKo: "프레수푸에스토",
    meaningKo: "예산",
    partOfSpeech: "명사",
    explanationKo: "사용 가능한 돈을 계획적으로 배분한 것",
    example: "Necesito ajustar mi presupuesto mensual.",
    examplePronunciationKo: "네세시토 아후스타르 미 프레수푸에스토 멘수알.",
    exampleKo: "월별 예산을 조정해야 한다.",
    exampleSegments: [
      { original: "Necesito ajustar mi presupuesto mensual.", ko: "월별 예산을 조정해야 한다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-035",
    language: "es",
    word: "horario",
    phonetic: "/oˈɾa.ɾjo/",
    pronunciationKo: "오라리오",
    meaningKo: "일정표",
    partOfSpeech: "명사",
    explanationKo: "활동이나 업무의 시간 계획표",
    example: "Mi horario de trabajo es muy ocupado.",
    examplePronunciationKo: "미 오라리오 데 트라바호 에스 무이 오쿠파도.",
    exampleKo: "내 업무 일정표는 매우 바쁘다.",
    exampleSegments: [
      { original: "Mi horario de trabajo ", ko: "내 업무 일정표는 " },
      { original: "es muy ocupado.", ko: "매우 바쁘다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-036",
    language: "es",
    word: "vecino",
    phonetic: "/beˈsi.no/",
    pronunciationKo: "베시노",
    meaningKo: "이웃",
    partOfSpeech: "명사",
    explanationKo: "근처에 사는 사람",
    example: "Mi vecino siempre me ayuda cuando lo necesito.",
    examplePronunciationKo: "미 베시노 시엠프레 메 아유다 쿠안도 로 네세시토.",
    exampleKo: "내 이웃은 내가 필요할 때 항상 도와준다.",
    exampleSegments: [
      { original: "Mi vecino siempre me ayuda cuando lo necesito.", ko: "내 이웃은 내가 필요할 때 항상 도와준다." },
    ],
    difficulty: "기초",
  },
  {
    id: "es-037",
    language: "es",
    word: "negociar",
    phonetic: "/ne.ɣoˈsjaɾ/",
    pronunciationKo: "네고시아르",
    meaningKo: "협상하다",
    partOfSpeech: "동사",
    explanationKo: "합의에 이르기 위해 서로 의견을 나누다",
    example: "Vamos a negociar los términos del acuerdo.",
    examplePronunciationKo: "바모스 아 네고시아르 로스 테르미노스 델 아쿠에르도.",
    exampleKo: "합의 조건에 대해 협상할 것이다.",
    exampleSegments: [
      { original: "Vamos a negociar los términos del acuerdo.", ko: "합의 조건에 대해 협상할 것이다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-038",
    language: "es",
    word: "ingresos",
    phonetic: "/iŋˈɡɾe.sos/",
    pronunciationKo: "잉그레소스",
    meaningKo: "수익",
    partOfSpeech: "명사",
    explanationKo: "사업 활동으로 벌어들이는 총 수입",
    example: "Los ingresos de la empresa aumentaron este año.",
    examplePronunciationKo: "로스 잉그레소스 데 라 엠프레사 아우멘타론 에스테 아뇨.",
    exampleKo: "회사의 수익이 올해 증가했다.",
    exampleSegments: [
      { original: "Los ingresos de la empresa ", ko: "회사의 수익이 " },
      { original: "aumentaron este año.", ko: "올해 증가했다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-039",
    language: "es",
    word: "plazo",
    phonetic: "/ˈpla.so/",
    pronunciationKo: "플라소",
    meaningKo: "마감일",
    partOfSpeech: "명사",
    explanationKo: "작업을 완료해야 하는 기한",
    example: "El plazo para entregar el proyecto es mañana.",
    examplePronunciationKo: "엘 플라소 파라 엔트레가르 엘 프로옉토 에스 마냐나.",
    exampleKo: "프로젝트 제출 마감일은 내일이다.",
    exampleSegments: [
      { original: "El plazo para entregar el proyecto ", ko: "프로젝트 제출 마감일은 " },
      { original: "es mañana.", ko: "내일이다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-040",
    language: "es",
    word: "agenda",
    phonetic: "/aˈxen.da/",
    pronunciationKo: "아헨다",
    meaningKo: "안건",
    partOfSpeech: "명사",
    explanationKo: "회의에서 논의할 주제의 목록",
    example: "Revisa la agenda antes de la reunión.",
    examplePronunciationKo: "레비사 라 아헨다 안테스 데 라 레우니온.",
    exampleKo: "회의 전에 안건을 검토해라.",
    exampleSegments: [
      { original: "Revisa la agenda antes de la reunión.", ko: "회의 전에 안건을 검토해라." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-041",
    language: "es",
    word: "propuesta",
    phonetic: "/pɾoˈpwes.ta/",
    pronunciationKo: "프로푸에스타",
    meaningKo: "제안서",
    partOfSpeech: "명사",
    explanationKo: "계획이나 아이디어를 공식적으로 제안한 문서",
    example: "Presentó una propuesta de negocio innovadora.",
    examplePronunciationKo: "프레센토 우나 프로푸에스타 데 네고시오 이노바도라.",
    exampleKo: "혁신적인 사업 제안서를 발표했다.",
    exampleSegments: [
      { original: "Presentó una propuesta de negocio innovadora.", ko: "혁신적인 사업 제안서를 발표했다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-042",
    language: "es",
    word: "fusión",
    phonetic: "/fuˈsjon/",
    pronunciationKo: "푸시온",
    meaningKo: "합병",
    partOfSpeech: "명사",
    explanationKo: "두 회사나 조직이 하나로 합쳐지는 것",
    example: "La fusión de las dos empresas fue un éxito.",
    examplePronunciationKo: "라 푸시온 데 라스 도스 엠프레사스 푸에 운 엑시토.",
    exampleKo: "두 회사의 합병은 성공적이었다.",
    exampleSegments: [
      { original: "La fusión de las dos empresas ", ko: "두 회사의 합병은 " },
      { original: "fue un éxito.", ko: "성공적이었다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-043",
    language: "es",
    word: "contrato",
    phonetic: "/konˈtɾa.to/",
    pronunciationKo: "콘트라토",
    meaningKo: "계약서",
    partOfSpeech: "명사",
    explanationKo: "양측이 서명한 법적 합의 문서",
    example: "Firmamos el contrato con el nuevo cliente.",
    examplePronunciationKo: "피르마모스 엘 콘트라토 콘 엘 누에보 클리엔테.",
    exampleKo: "새 고객과 계약서에 서명했다.",
    exampleSegments: [
      { original: "Firmamos el contrato con el nuevo cliente.", ko: "새 고객과 계약서에 서명했다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-044",
    language: "es",
    word: "accionista",
    phonetic: "/ak.sjoˈnis.ta/",
    pronunciationKo: "악시오니스타",
    meaningKo: "주주",
    partOfSpeech: "명사",
    explanationKo: "회사의 주식을 소유한 사람",
    example: "Los accionistas aprobaron el nuevo plan estratégico.",
    examplePronunciationKo: "로스 악시오니스타스 아프로바론 엘 누에보 플란 에스트라테히코.",
    exampleKo: "주주들이 새 전략 계획을 승인했다.",
    exampleSegments: [
      { original: "Los accionistas aprobaron el nuevo plan estratégico.", ko: "주주들이 새 전략 계획을 승인했다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-045",
    language: "es",
    word: "itinerario",
    phonetic: "/i.ti.neˈɾa.ɾjo/",
    pronunciationKo: "이티네라리오",
    meaningKo: "여행 일정",
    partOfSpeech: "명사",
    explanationKo: "여행 중 방문할 장소와 시간을 정리한 계획",
    example: "Nuestro itinerario incluye tres países distintos.",
    examplePronunciationKo: "누에스트로 이티네라리오 잉클루예 트레스 파이세스 디스틴토스.",
    exampleKo: "우리 여행 일정에는 세 개의 나라가 포함된다.",
    exampleSegments: [
      { original: "Nuestro itinerario incluye tres países distintos.", ko: "우리 여행 일정에는 세 개의 나라가 포함된다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-046",
    language: "es",
    word: "alojamiento",
    phonetic: "/a.lo.xaˈmjen.to/",
    pronunciationKo: "알로하미엔토",
    meaningKo: "숙박",
    partOfSpeech: "명사",
    explanationKo: "여행 중 머물 장소나 시설",
    example: "Reservamos un alojamiento cerca del centro.",
    examplePronunciationKo: "레세르바모스 운 알로하미엔토 세르카 델 센트로.",
    exampleKo: "도심 근처의 숙박 시설을 예약했다.",
    exampleSegments: [
      { original: "Reservamos un alojamiento cerca del centro.", ko: "도심 근처의 숙박 시설을 예약했다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-047",
    language: "es",
    word: "aduana",
    phonetic: "/aˈðwa.na/",
    pronunciationKo: "아두아나",
    meaningKo: "세관",
    partOfSpeech: "명사",
    explanationKo: "입국 시 반입 물품을 검사하는 공식 기관",
    example: "Tuvimos que pasar por la aduana al llegar.",
    examplePronunciationKo: "투비모스 케 파사르 포르 라 아두아나 알 예가르.",
    exampleKo: "도착 시 세관을 통과해야 했다.",
    exampleSegments: [
      { original: "Tuvimos que pasar por la aduana al llegar.", ko: "도착 시 세관을 통과해야 했다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-048",
    language: "es",
    word: "salida",
    phonetic: "/saˈli.ða/",
    pronunciationKo: "살리다",
    meaningKo: "출발",
    partOfSpeech: "명사",
    explanationKo: "어떤 장소를 떠나는 행동",
    example: "La salida del vuelo es a las ocho de la mañana.",
    examplePronunciationKo: "라 살리다 델 부엘로 에스 아 라스 오초 데 라 마냐나.",
    exampleKo: "항공편 출발은 오전 8시이다.",
    exampleSegments: [
      { original: "La salida del vuelo ", ko: "항공편 출발은 " },
      { original: "es a las ocho de la mañana.", ko: "오전 8시이다." },
    ],
    difficulty: "중급",
  },
  {
    id: "es-049",
    language: "es",
    word: "recuerdo",
    phonetic: "/reˈkweɾ.ðo/",
    pronunciationKo: "레쿠에르도",
    meaningKo: "기념품",
    partOfSpeech: "명사",
    explanationKo: "여행지에서 기억을 남기기 위해 사는 물건",
    example: "Compré un recuerdo para toda mi familia.",
    examplePronunciationKo: "콤프레 운 레쿠에르도 파라 토다 미 파밀리아.",
    exampleKo: "가족 모두를 위해 기념품을 샀다.",
    exampleSegments: [
      { original: "Compré un recuerdo para toda mi familia.", ko: "가족 모두를 위해 기념품을 샀다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-050",
    language: "es",
    word: "monumento",
    phonetic: "/mo.nuˈmen.to/",
    pronunciationKo: "모누멘토",
    meaningKo: "기념물",
    partOfSpeech: "명사",
    explanationKo: "역사적으로 중요하거나 유명한 건물이나 구조물",
    example: "Visitamos el monumento histórico de la ciudad.",
    examplePronunciationKo: "비시타모스 엘 모누멘토 이스토리코 데 라 시우다드.",
    exampleKo: "도시의 역사적 기념물을 방문했다.",
    exampleSegments: [
      { original: "Visitamos el monumento histórico de la ciudad.", ko: "도시의 역사적 기념물을 방문했다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-051",
    language: "es",
    word: "posteriormente",
    phonetic: "/pos.te.ɾjoɾˈmen.te/",
    pronunciationKo: "포스테리오르멘테",
    meaningKo: "그 후에",
    partOfSpeech: "부사",
    explanationKo: "어떤 사건이나 행동이 일어난 이후에",
    example: "El informe fue revisado y aprobado posteriormente.",
    examplePronunciationKo: "엘 인포르메 푸에 레비사도 이 아프로바도 포스테리오르멘테.",
    exampleKo: "보고서는 검토된 후 승인되었다.",
    exampleSegments: [
      { original: "El informe fue revisado ", ko: "보고서는 검토된 후 " },
      { original: "y aprobado posteriormente.", ko: "승인되었다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-052",
    language: "es",
    word: "compensar",
    phonetic: "/kom.penˈsaɾ/",
    pronunciationKo: "콤펜사르",
    meaningKo: "보상하다",
    partOfSpeech: "동사",
    explanationKo: "손실이나 피해에 대해 대가를 지불하다",
    example: "La empresa compensará a los empleados afectados.",
    examplePronunciationKo: "라 엠프레사 콤펜사라 아 로스 엠플레아도스 아펙타도스.",
    exampleKo: "회사는 피해를 입은 직원들에게 보상할 것이다.",
    exampleSegments: [
      { original: "La empresa compensará a los empleados afectados.", ko: "회사는 피해를 입은 직원들에게 보상할 것이다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-053",
    language: "es",
    word: "implementar",
    phonetic: "/im.ple.menˈtaɾ/",
    pronunciationKo: "임플레멘타르",
    meaningKo: "시행하다",
    partOfSpeech: "동사",
    explanationKo: "계획이나 결정을 실제로 실행하다",
    example: "Debemos implementar la nueva política de inmediato.",
    examplePronunciationKo: "데베모스 임플레멘타르 라 누에바 폴리티카 데 인메디아토.",
    exampleKo: "새 정책을 즉시 시행해야 한다.",
    exampleSegments: [
      { original: "Debemos implementar la nueva política de inmediato.", ko: "새 정책을 즉시 시행해야 한다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-054",
    language: "es",
    word: "factura",
    phonetic: "/fakˈtu.ɾa/",
    pronunciationKo: "팍투라",
    meaningKo: "청구서",
    partOfSpeech: "명사",
    explanationKo: "제공된 서비스나 상품에 대한 청구 명세서",
    example: "Por favor, envíe la factura antes del viernes.",
    examplePronunciationKo: "포르 파보르, 엔비에 라 팍투라 안테스 델 비에르네스.",
    exampleKo: "금요일 전까지 청구서를 보내주세요.",
    exampleSegments: [
      { original: "Por favor, envíe la factura antes del viernes.", ko: "금요일 전까지 청구서를 보내주세요." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-055",
    language: "es",
    word: "preliminar",
    phonetic: "/pɾe.li.miˈnaɾ/",
    pronunciationKo: "프렐리미나르",
    meaningKo: "예비의",
    partOfSpeech: "형용사",
    explanationKo: "본 단계 전에 먼저 이루어지는",
    example: "Los resultados preliminares son muy positivos.",
    examplePronunciationKo: "로스 레술타도스 프렐리미나레스 손 무이 포시티보스.",
    exampleKo: "예비 결과가 매우 긍정적이다.",
    exampleSegments: [
      { original: "Los resultados preliminares ", ko: "예비 결과가 " },
      { original: "son muy positivos.", ko: "매우 긍정적이다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-056",
    language: "es",
    word: "fiscal",
    phonetic: "/fisˈkal/",
    pronunciationKo: "피스칼",
    meaningKo: "재정의",
    partOfSpeech: "형용사",
    explanationKo: "정부나 기업의 재정이나 세금에 관한",
    example: "El año fiscal de la empresa termina en diciembre.",
    examplePronunciationKo: "엘 아뇨 피스칼 데 라 엠프레사 테르미나 엔 디시엠브레.",
    exampleKo: "회사의 회계 연도는 12월에 끝난다.",
    exampleSegments: [
      { original: "El año fiscal de la empresa ", ko: "회사의 회계 연도는 " },
      { original: "termina en diciembre.", ko: "12월에 끝난다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-057",
    language: "es",
    word: "adquirir",
    phonetic: "/að.kiˈɾiɾ/",
    pronunciationKo: "아드키리르",
    meaningKo: "획득하다",
    partOfSpeech: "동사",
    explanationKo: "노력이나 비용을 들여 무언가를 얻다",
    example: "La empresa planea adquirir nuevas tecnologías.",
    examplePronunciationKo: "라 엠프레사 플라네아 아드키리르 누에바스 테크놀로히아스.",
    exampleKo: "회사는 새 기술을 획득할 계획이다.",
    exampleSegments: [
      { original: "La empresa planea adquirir nuevas tecnologías.", ko: "회사는 새 기술을 획득할 계획이다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-058",
    language: "es",
    word: "subsidiaria",
    phonetic: "/sub.siˈðja.ɾja/",
    pronunciationKo: "숩시디아리아",
    meaningKo: "자회사",
    partOfSpeech: "명사",
    explanationKo: "더 큰 회사가 통제하는 소규모 회사",
    example: "La subsidiaria opera de forma independiente.",
    examplePronunciationKo: "라 숩시디아리아 오페라 데 포르마 인데펜디엔테.",
    exampleKo: "자회사는 독립적으로 운영된다.",
    exampleSegments: [
      { original: "La subsidiaria opera de forma independiente.", ko: "자회사는 독립적으로 운영된다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-059",
    language: "es",
    word: "gasto",
    phonetic: "/ˈɡas.to/",
    pronunciationKo: "가스토",
    meaningKo: "지출",
    partOfSpeech: "명사",
    explanationKo: "특정 목적을 위해 사용한 돈",
    example: "El gasto anual en formación ha aumentado.",
    examplePronunciationKo: "엘 가스토 아누알 엔 포르마시온 아 아우멘타도.",
    exampleKo: "교육 관련 연간 지출이 증가했다.",
    exampleSegments: [
      { original: "El gasto anual en formación ", ko: "교육 관련 연간 지출이 " },
      { original: "ha aumentado.", ko: "증가했다." },
    ],
    difficulty: "고급",
  },
  {
    id: "es-060",
    language: "es",
    word: "reconocer",
    phonetic: "/re.ko.noˈseɾ/",
    pronunciationKo: "레코노세르",
    meaningKo: "인정하다",
    partOfSpeech: "동사",
    explanationKo: "사실이나 상황의 존재를 공식적으로 인정하다",
    example: "Reconoció su error ante toda la empresa.",
    examplePronunciationKo: "레코노시오 수 에로르 안테 토다 라 엠프레사.",
    exampleKo: "그는 회사 전체 앞에서 자신의 실수를 인정했다.",
    exampleSegments: [
      { original: "Reconoció su error ante toda la empresa.", ko: "그는 회사 전체 앞에서 자신의 실수를 인정했다." },
    ],
    difficulty: "고급",
  },
];
